...
def LSTM_Model(X_train, Y_train, model_name, i):
    ...
    '''
    # , EarlyStopping(monitor='val_loss', patience=10, min_delta=0.0000001)
    model.add(Dense(1))
...
if __name__ == '__main__':
    ...
    # do = df['氨氮(mg/L)']  # 返回溶解氧那一列，用字典的方式
    do = df[data_name]  # 返回溶解氧那一列，用字典的方式
    ...
    if train_mode == 'train':
        ...
        for imf in imfs:
            ...
            Y_test_true += Y2_test
            model, history = LSTM_Model(X2_train, Y2_train, model_name, i)
        ...
        for imf in imfs:
            ...
            imfs_prediction.append(prediction_Y)
            i += 1
    # Y_test_true1 = data_split_2(imf_data(DO, 1), c, lookback_window)
    ...
    #     plot_curve(Y_test_true[i:i + 50], Y_test_prediction[i:i + 50], rmse, mae, mape, r2, i, f'{train_mode}_{i}',
    #                model_name)
