...
from Code.utils import plot_curve, data_split, isolutionforest, imf_data, data_split_LSTM, RMSE, MAPE, data_split_1, \
    data_split_LSTM_1, plot_curve_1, data_split_6
...
def LSTM_Model(X_train, Y_train, i):
    os.makedirs('../checkpoints/EEMD_LSTM', exist_ok=True)
    filepath = "../checkpoints/EEMD_LSTM/EEMD_LSTM_imf" + str(i) + "_{epoch:02d}.h5"
...
if __name__ == '__main__':
    ...
    os.makedirs('../pictures/EEMD_LSTM', exist_ok=True)
    plt.savefig('../pictures/EEMD_LSTM/result_imf.png')

    Y_test_true = np.zeros([len(DO) - c - lookback_window - n_out + 1, 1])
    ...
    if train_mode:
        ...
        for imf in imfs:
            ...
            print('*' * 45)
            X1_train, Y1_train, X1_test, Y1_test = data_split_6(imf_data(imf, 1), c, lookback_window, n_out)
        ...
        for imf in imfs:
            ...
            Y_test_true += Y2_test
            model = load_model('../checkpoints/EEMD_LSTM/EEMD_LSTM_imf' + str(i) + '_100.h5')
            prediction_Y = model.predict(X2_test)
    ...
    plot_curve_1(Y_test_true[:, 0], Y_test_prediction[:, 0], 5, 'test', 'EEMD_LSTM_6', 1)

    print('RMSE:' + str(rmse) + '\n' + 'MAE:' + str(mae) + '\n' + 'MAPE:' + str(mape) + '\n' + 'R2:' + str(r2))
    eval_indicator = pd.DataFrame({'RMSE': rmse, 'MAE': mae, 'MAPE': mape, 'R2': r2}, index=['values'])
    eval_indicator.to_csv('../pictures/EEMD_LSTM/eval_indicator.csv', index=False)
