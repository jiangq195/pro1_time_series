...
from Code.utils import plot_curve, isolutionforest, data_split, imf_data, data_split_LSTM, RMSE, MAPE
...
def LSTM_Model(X_train, Y_train, i):
    os.makedirs('../checkpoints/Sever_train_version_1', exist_ok=True)
    filepath = "../checkpoints/Sever_train_version_1/Sever_train_version_1_imf" + str(i) + "_{epoch:02d}.h5"
...
if __name__ == '__main__':
    ...
    os.makedirs('../pictures/Sever_train_version_1', exist_ok=True)
    plt.savefig('../pictures/Sever_train_version_1/result_imf.png')
    ...
    if train_mode:
        ...
        for imf in imfs:
            ...
            Y_test_true += Y2_test
            model = load_model(
                "../checkpoints/Sever_train_version_1/Sever_train_version_1_imf" + str(i) + "_{epoch:02d}.h5")
            prediction_Y = model.predict(X2_test)
            imfs_prediction.append(prediction_Y)
            i += 1
    ...
    plot_curve(Y_test_true, Y_test_prediction, rmse, mae, mape, r2, 5, 'test', 'Sever_train_version_1')
    print('RMSE:' + str(rmse) + '\n' + 'MAE:' + str(mae) + '\n' + 'MAPE:' + str(mape) + '\n' + 'R2:' + str(r2))
    eval_indicator = pd.DataFrame({'RMSE': rmse, 'MAE': mae, 'MAPE': mape, 'R2': r2}, index=['values'])
    eval_indicator.to_csv('../pictures/Sever_train_version_1/eval_indicator.csv', index=False)
